...
void loop() {
  ...
  Serial.println("");  
  if(temp>28){                                      // If temperature is above 28C, turn red (hot)
    setColor(255, 0, 0);  // red
  }
  else if (temp>27){
    setColor(255, 255, 0);//yellow                  // If temperature is above 27C, turn yellow (warm)
  }
  else if (temp>26){                                // If temperature is above 26C, turn green (room temp)
    setColor(0, 255, 0);//green
  }
  else {                                            // Otherwise (when temp is below 26), turn blue (cool)
